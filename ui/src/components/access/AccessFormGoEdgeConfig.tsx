...
const AccessFormGoEdgeConfig = ({ form: formInst, formName, disabled, initialValues, onValuesChange }: AccessFormGoEdgeConfigProps) => {
  ...
  const formSchema = z.object({
    ...
    userId: z
      .string()
      .min(1, t("access.form.goedge_user_id.placeholder"))
  ...
  return (
    ...
    >
      ...
      <Form.Item
        name="userId"
        label={t("access.form.goedge_user_id.label")}
        rules={[formRule]}
        tooltip={<span dangerouslySetInnerHTML={{ __html: t("access.form.goedge_user_id.tooltip") }}></span>}
      >
        <Input type="number" placeholder={t("access.form.goedge_user_id.placeholder")} />
      </Form.Item>
